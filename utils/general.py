...
def get_coords(txt):
    ...
    return np.array(box_coords_true)
...
def extract_from_txt(txt, task='train/'):
    ...
    img = cv2.imread('./dataset/images/' + task + img_name + '.png')
...
def convert_to_yolov5(info_dict, task):
    ...
    if info_dict["bboxes"]:
        ...
        for b in info_dict["bboxes"]:
            ...
            print_buffer.append(
                "{} {:.3f} {:.3f} {:.3f} {:.3f}".format(class_id, min(b_center_x, 1.), min(b_center_y, 1.), b_width,
                                                        b_height))

    if not os.path.isdir('./dataset/labels/' + task):
        os.mkdir('./dataset/labels/' + task)

    # Name of the file which we have to save
    save_file_name = os.path.join("./dataset/labels", task, info_dict["filename"].replace("png", "txt"))

    # Save the annotation to disk
    print("\n".join(print_buffer), file=open(save_file_name, "w"))


def generate_json_coco(all_dataset):
    json_dict = {'categories': [{'id': 0, 'name': 'infant seat', 'supercategory': ''},
                                {'id': 1, 'name': 'child seat', 'supercategory': ''},
                                {'id': 2, 'name': 'person', 'supercategory': ''},
                                {'id': 3, 'name': 'everyday object', 'supercategory': ''}],
                 'images': [],
                 'annotations': []}

    id_anno = 0
    id_count = 0
    for i, data in enumerate(tqdm(all_dataset, desc='Generating Json')):
        height, width = 640, 960
        file_name = data.split('/')[-1].split('.')[0]
        json_dict['images'].append({'id': id_count,
                                    'file_name': file_name,
                                    'width': width,
                                    'height': height})
        lab_name = data.replace('images', 'labels_xyxy').replace('.png', '.txt')
        read_label = pd.read_table(lab_name, names=['info']).values.tolist()
        for row in read_label:
            label, x1, y1, x2, y2 = list(map(int, row[0].split(',')))
            json_dict['annotations'].append({'id': id_anno,
                                             'image_id': id_count,
                                             'category_id': label,
                                             'bbox': [x1*0.666666, y1, x2*0.666666, y2],
                                             'area': (y2 - y1) * (x2*0.666666 - x1*0.666666),
                                             'iscrowd': 0})
            id_anno += 1
        id_count += 1
...
def save_one_json(predn, jdict, path, data_json, all_ids):
    # Save one JSON result {"image_id": 42, "category_id": 18, "bbox": [258.15, 41.29, 348.26, 243.78], "score": 0.236}
    for i in data_json['images']:
        if i['file_name'] == path.split('/')[-1].split('.')[0]:
            image_id = i['id']
            continue
