...
def plot_image_cropped_new(pic, bboxes):
    ...
    if not np.array(bboxes).any():
        return img_
    for box in np.array(bboxes):
        label, x1, y1, x2, y2 = list(map(int, box))
        x = [x1, y1, x2, y2]
        plot_one_box(x, img_, color=color_dict[label], label=labels_dict[label], line_thickness=3)
    return img_
